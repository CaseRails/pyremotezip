...
class RemoteZip(object):
    ...
    def getTableOfContents(self):
        ...
        local_raw_bytes = self.raw_bytes
        ...
        try:
            while True:
                ...
                zip_n = unpack("H", local_raw_bytes[current_start + 28: current_start + 28 + 2])[0]
                zip_m = unpack("H", local_raw_bytes[current_start + 30: current_start + 30 + 2])[0]
                zip_k = unpack("H", local_raw_bytes[current_start + 32: current_start + 32 + 2])[0]

                filename = local_raw_bytes[current_start + 46: current_start + 46 + zip_n]

                # check if this is the index file
                filestart = unpack("I", local_raw_bytes[current_start + 42: current_start + 42 + 4])[0]
                compressedsize = unpack("I", local_raw_bytes[current_start + 20: current_start + 20 + 4])[0]
                uncompressedsize = unpack("I", local_raw_bytes[current_start + 24: current_start + 24 + 4])[0]
                tableItem = {
                    'filename': filename,
